...
const SkillSets: FC<ISkillSetsProps> = (props) => {
    ...
    const getSkills = async () => {
        ...
        try {
            ...
            //     mapSkillsetData.skill_id = skillsetRow.skill_id;
            //     mapSkillsetData.skill_name = skillsetRow.skill_name;
            //     mapSkillsetData.employees = skillsetRow.employees
            //     mapSkillsetDataArray.push(mapSkillsetData)
            // })
                                    ...
                                    <tr key={Math.random()}>
                                        ...
                                        {/* <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                            {row?.employees ? row?.employees.map((employee) => {
                                                return (
                                                    <i key={Math.random()} className="block fas fa-arrow-up text-emerald-500 mr-4">{employee}</i>
                                                )
                                            }) : '...'}
                                        </td> */}
                                        ...
                                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 flex items-center">
                                            ...
                                            {row?.skill_id &&
                                                <button onClick={(e) => handleDelete(row?.skill_id)} className='flex text-3xl text-red-500'><MdOutlineDeleteSweep /></button>}
                                        </td>
