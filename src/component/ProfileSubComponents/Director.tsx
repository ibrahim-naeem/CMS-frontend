...
const Director: FC<IDirectorProps> = (props) => {
  ...
  const [allDirector, setAllDirector] = useState<any>([]);
  ...
  const getAllDirectors = async () => {
    ...
    const response = await fetch("http://localhost:5000/user/getAllDirector", {
      headers: { token },
    });
    ...
    res.map((director) => {
      setAllDirector((prevState) => [...prevState, director]);
    });
  };
                  ...
                  return (
                    <tr key={Math.random()}>
                      ...
                      <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 flex items-center">
                        ...
                        {row?.director_id && (
                          <button
                            onClick={(e) => handleDelete(row?.director_id)}
